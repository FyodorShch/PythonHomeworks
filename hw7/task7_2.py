...
num_rows = int(input())
num_collumns = int(input())


def print_operation_table(operation, num_rows, num_collumns):
    for row in range(1, num_rows+1):
        row_str = f"{row}\t"

        for col in range(1, num_collumns+1):
            row_str += f"{operation(row, col)}\t"

        print(row_str)


print(print_operation_table(lambda x, y: x**y, num_rows, num_collumns))
